...
export function getAvailableRoutes(user: User | null): RoleBasedRoute[] {
  ...
  // Editor gets access to ONLY editor and user routes (restricted access)
...
/**
 * Check if user can access admin dashboard (excluding editors)
 */
export function canAccessAdminDashboard(user: User | null): boolean {
  ...
  // Editors cannot access admin dashboard
  if (user.roles.some(role => role.slug === 'editor')) {
    return false;
  }

  return user.roles.some(role => 
    ['super-admin', 'admin', 'manager'].includes(role.slug)
  );
}
 ...
 */
export function canAccessSuperAdminDashboard(user: User | null): boolean {
  if (!user || !user.roles || user.roles.length === 0) {
    return false;
  }
